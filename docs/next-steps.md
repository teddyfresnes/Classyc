...
- La progression bleue de la route campagne calcule les paliers sur la portion visible entre deux pastilles : la route cachee sous les niveaux et le bout rond du trait sont compenses, et un niveau a `0/3` ne fait pas avancer le segment bleu.
- Correction feedback exercices appliquee le 2026-06-24 : les choix image illustrables montrent le mot en langue apprise et des illustrations en reponse, les traductions a relier gardent la langue utilisateur a gauche et la langue apprise a droite, et les conversations `How are you?` / `Ca va ?` evitent les distracteurs `oui`/`non` au profit d'une seule bonne reponse.
- Le design conversation utilise une bulle question a gauche et une grande bulle reponses a droite, sans l'ancien panneau de thread autour de la question.
- Reprise feedback exercices appliquee le 2026-06-24 : la bulle reponses reprend le meme style que la bulle question ; les choix image utilisent `Choisis la bonne reponse pour <mot>` dans le titre sans grande carte mot ; certaines traductions simples proposent un mot en langue utilisateur avec des options en langue apprise ; les ordres de mots ont 2-3 mots pieges et plus de placeholder ; les questions de lecture `Quel mot veut dire <mot>` mettent seulement le mot cle en accent texte primaire.
...
- Verification correction exercices 2026-06-24 : `npm run typecheck`, `npm run lint` et `npm run build` OK. Build avec l'avertissement Vite connu sur le bundle volumineux.
- Verification Chrome headless/CDP 2026-06-24 : choix image `Hello` sans image de prompt et avec 3 options OpenMoji, matching `Bonjour/Merci/Au revoir` a gauche et `Hello/Thank you/Goodbye` a droite, conversation sans thread panel, sans `Yes`/`No`, et sans overflow desktop/mobile.
- Verification reprise feedback exercices 2026-06-24 : `npm run typecheck`, `npm run lint`, `npm run build` OK ; Chrome headless/CDP confirme titre image inline, variante traduction inverse, mots pieges en ordre des mots, bulles conversation avec meme fond/radius, lecture avec mot cle accentue et mobile sans overflow.
